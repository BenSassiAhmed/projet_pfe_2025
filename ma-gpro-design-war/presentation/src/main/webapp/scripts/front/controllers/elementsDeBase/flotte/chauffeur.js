angular
		.module('gpro.chauffeur', [ "ngResource" ])
		.controller(
				'ChauffeurController',
				[
						'$rootScope',
						...
						function($rootScope, $scope, $filter, $http, $log,
							...
							$scope.sauvegarderBonVente = function(marche) {
								...
								if (angular.isDefined(marche.id)) {
									$log.debug("Sauvegarder Modification : "+ marche.matricule);
							...
							$scope.creerMarche = function(marche) {
								console.log("----Marche id  : "+marche.matricule);

								
								
								$http
										.post(UrlAtelier + "/personnel/create",
												marche)
										.success(
												function(marcheId) {
