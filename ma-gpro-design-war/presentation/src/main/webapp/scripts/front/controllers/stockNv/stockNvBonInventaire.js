'use strict'
angular
	.module('gpro.stockNvBonInventaire', [])
	.controller(
		'stockNvBonInventaireController',
		[
			'$scope',
			'$http',
			'$filter',
			...
			function ($scope, $http, $filter, $rootScope, $log,
				 ...
				 * 
				 * $http .get( UrlAtelier +
				 * "/bonInventaire/validerFactureAvoirRetour:" + ref) .success(
				 * function (bonInventaire) {
				 * 
				 * if (bonInventaire.refAvoirRetour == null) {
				 * 
				 * $scope.refAvoirRetourIsInvalid = true;
				 *  } else { $scope.validerAvoirRetourSucess = true;
				 * $scope.bonInventaireCourante.dateConcernee =
				 * bonInventaire.dateConcernee; $scope.listDetBonInventaire =
				 * bonInventaire.listDetBonInventaire;
				 * $scope.bonInventaireCourante.partieIntId =
				 * bonInventaire.partieIntId;
				 * 
				 * $scope.bonInventaireCourante.refFacture =
				 * bonInventaire.refFacture;
				 *  }
				 * 
				 * 
				 ...
				 * $scope.isEnCoursValider = true;
				 * 
				 * $http .get( UrlAtelier + "/bonInventaire/validerByRefBR:" +
				 * ref) .success( function (bonInventaireResponse) {
				 * 
				 * if (bonInventaireResponse.refBR == null) {
				 * 
				 * $scope.refBLIsInvalid = true;
				 *  } else { $scope.validerBLSucess = true;
				 * $scope.bonInventaireCourante.dateConcernee =
				 * bonInventaireResponse.dateConcernee;
				 * $scope.listDetBonInventaire =
				 * bonInventaireResponse.listDetBonInventaire;
				 * $scope.bonInventaireCourante.partieIntId =
				 * bonInventaireResponse.partieIntId; }
				 * 
				 * 
				 * $scope.isEnCoursValider = false;
				 * 
				 ...
				 * $scope.listReferenceBC = []; $scope.listeMarche = [];
				 * 
				 * if (angular.isDefined(idClient)) { if (idClient != null) {
				 * 
				 * 
				 * 
				 * 
				 * $http .get( UrlAtelier +
				 * "/commandeAchat/getAvailableListBonCommandeRefByFournisseur:" +
				 * idClient) .success( function (resultat) { //
				...
				$scope.ajouterNumSerie = function (numero, index) {
					...
					if ($scope.listDetBonInventaire[index].produitsSerialisable) {

						var produitSerialisable = { numSerie: numero };

						// if
						// (!$scope.listDetBonInventaire[index].produitsSerialisable.some(e
						// => e.numSerie === numero))

						if ($scope.listDetBonInventaire[index].produitsSerialisable.filter(e => e.numSerie === numero).length == 0)
							 {
						     	$scope.listDetBonInventaire[index].produitsSerialisable.push(produitSerialisable);
						     	$scope.numSerie = "";
						...
						$scope.listDetBonInventaire[index].produitsSerialisable = [];
						var produitSerialisable = { numSerie: numero };
				...
				$scope.isValidSauvegarde = function () {
					...
					angular.forEach($scope.listDetBonInventaire, function (element, key) {
				...
				$scope.removeNumSerie = function (index, indexPS) {
					...
					$scope.listDetBonInventaire[index].produitsSerialisable.splice(indexPS, 1);
				...
				$scope.cancelAddReceptionAchat = function (rowform,
					...
					if (angular.isDefined(id)) {
						...
						$scope.listDetBonInventaire.splice(index,
							1);
						rowform.$cancel();
					}
					$scope.deleteValue = "oui";
				...
				$scope.AffectationAchatBC = function () {
					$scope.validerAvoirRetourSucess = false;
					$scope.bonInventaireCourante = { "date": new Date() };
					$scope.listTaxeReceptionAchat = [
						/*
						 * {//FODEC taxeId: 1, pourcentage: 1, montant: '', },
						 */
					...
					$scope.bonInventaireCourante.idDepot = $scope.listeMagazinCache.length > 0 ? $scope.listeMagazinCache[0].id : '';


					// $scope.bonInventaireCourante = reception ?
					// angular
					// .copy(reception) : {};
					...
					$http
						.get(
							UrlAtelier
							+ "/bonInventaire/getCurrentReference"
						)
						.success(
							function (data) {
								$scope.bonInventaireCourante.reference = data;
								$scope.bonInventaireCourante.refAvantChangement = data;
				...
				$scope.rechercherReceptionAchat = function (
					...
					$http
						.post(
							UrlAtelier
							+ "/bonInventaire/rechercheMulticritere",
							bonInventaireCourante)
						.success(
							function (data) {
				...
				$scope.downloadAllReceptionAchatExcel = function (
					...
					$http({
						url: UrlAtelier + "/fichesGS/listBonInventaire",
						method: "POST",
						...
						var fileName = 'Bon_Inventaire_' + formattedDate(new Date());
				...
				$scope.downloadBonInventaire =function(bonInventaireCourante){
					var url = UrlAtelier
					+ "/fichesGS/bonInventaire?id=" + bonInventaireCourante.id;
					...
					$http
						...
						.success(
							function (datagetReceptionAchat) {
								 ...
								 */
								// produitReception
								$scope.listDetBonInventaire = datagetReceptionAchat.listDetBonInventaire;
								...
								$scope.myDataBonInventaire[index].listProduitReceptions = $scope.listDetBonInventaire;
							...
							if ($scope.myDataBonInventaire[index].date !== null) {
								dateInv = $scope.modifierFormatDate($scope.myDataBonInventaire[index].date);
							} else {
								dateInv = null;
							}
							...
							$scope.bonInventaireCourante = Object.assign($scope.myDataBonInventaire[index], { date: dateInv })
							//  $scope.partieInteresseeCourante = $scope.myData[index]
							? angular.copy($scope.myDataBonInventaire[index])
							: {};

				/*	$scope.bonInventaireCourante = $scope.myDataBonInventaire[index] ? angular
				...
				$scope.creerReceptionAchat = function (reception) {
					...
					reception.listDetBonInventaire = $scope.listDetBonInventaire;
								...
								$http
									...
									.success(
										function (
											...
											$scope.bonInventaireCourante = datagetReceptionAchat ? angular
				...
				$scope.updateReceptionAchat = function (reception) {
					...
					reception.listDetBonInventaire.serialisable = true;
				...
				$scope.clickProduit = function (idProduit, index) {
					...
					$scope.updateProduitCommandDetails(idProduit,
						$scope.listDetBonInventaire[index]);
				...
				$scope.removeProduit = function (index) {
					$scope.listSuppDetBonInventaire.push($scope.listDetBonInventaire[index]);
					$scope.listDetBonInventaire.splice(index, 1);
				...
				$scope.getPagedDataAsync = function (pageSize, page,
					...
					setTimeout(
						function () {
							var data;
							var bonInventaireCourante = $scope.bonInventaireCourante;
							bonInventaireCourante.type = 'Sortie';
							if (searchText) {
								var ft = searchText
									.toLowerCase();
