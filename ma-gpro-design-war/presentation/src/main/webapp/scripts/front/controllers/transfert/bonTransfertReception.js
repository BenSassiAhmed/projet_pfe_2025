'use strict'
angular
	.module('gpro.transfertBonTransfertReception', [])
	.controller(
		'transfertBonTransfertReceptionController',
		[
			'$scope',
			'$http',
			'$filter',
			...
			function ($scope, $http, $filter, $rootScope, $log,
				...
				const CODE_ERROR_CREATION_BON_TRANSFERT_RECEPTION_INVALIDE_STOCK_IN_MAGASIN_ORIGINE = "CODE_ERROR_CREATION_BON_TRANSFERT_RECEPTION_INVALIDE_STOCK_IN_MAGASIN_ORIGINE";
				...
				$scope.validerBT = function(refSortie){
					
					$scope.isEnCoursValider = true;
					
					$http.get(
							UrlAtelier
							+ "/bonTransfert/validerBTsortieByReference:"
							+ refSortie)
							.success(
						function (bt) {
							...
							if(bt != ''){
								...
								$scope.bonTransfertReceptionCourante.idDepotOrigine = bt.idDepotOrigine;
								$scope.bonTransfertReceptionCourante.idDepotDestination = bt.idDepotDestination;
								
								$scope.listDetBonTransfert = [];
								
								
								//$scope.listDetBonTransfert = bt.listDetBonTransfert;
								
								
								angular.forEach(bt.listDetBonTransfert, function (element, key) {

									element.id = null;
									
									element.quantiteEnvoyer = element.quantite;

									$scope.listDetBonTransfert.push(element);


								});
								
								
								$scope.validerBTSucess = true;
				...
				$scope.AffectationAchatBC = function () {
					$scope.validerAvoirRetourSucess = false;
					$scope.bonTransfertReceptionCourante = { "date": new Date() };
					$scope.listTaxeReceptionAchat = [
						/*
						 * {//FODEC taxeId: 1, pourcentage: 1, montant: '', },
						 */
					...
					$scope.bonTransfertReceptionCourante.idDepot = $scope.listeMagazinCache.length > 0 ? $scope.listeMagazinCache[0].id : '';


					// $scope.bonTransfertReceptionCourante = reception ?
					// angular
					// .copy(reception) : {};
					...
					$http
						.get(
							UrlAtelier
							+ "/bonTransfert/getCurrentReferenceByType:Reception"
						)
						.success(
							function (data) {
								$scope.bonTransfertReceptionCourante.reference = data;
								$scope.bonTransfertReceptionCourante.refAvantChangement = data;
				...
				$scope.rechercherReceptionAchat = function (
					...
					bonInventaireCourante.type = 'Reception';
					$http
						.post(
							UrlAtelier
				...
				$scope.downloadAllReceptionAchatExcel = function (
					...
					$http({
						...
						var fileName = 'List_Bon_Transfert_Reception_' + formattedDate(new Date());
				...
				$scope.modifierOuCreerReceptionAchat = function () {
					...
					$scope.bonTransfertReceptionCourante = $scope.myDataBonInventaire[index] ? angular
				...
				$scope.creerReceptionAchat = function (reception) {
					...
					$http
						...
						.success(
							...
							if(newReceptionId==CODE_ERROR_CREATION_BON_TRANSFERT_RECEPTION_INVALIDE_STOCK_IN_MAGASIN_ORIGINE)
								{
								    
								  $scope.codeErrorCreation = CODE_ERROR_CREATION_BON_TRANSFERT_RECEPTION_INVALIDE_STOCK_IN_MAGASIN_ORIGINE;
								  return;
								}
								...
								$http
									...
									.success(
										function (
											...
											$scope.bonTransfertReceptionCourante = datagetReceptionAchat ? angular
					...
					.$watch(
						...
						function () {
							$scope.colDefs = [
								...
								{
									field: 'referenceSortie',
									displayName: 'Reference Sortie',
									// width : '10%'
								},
								{
									field: 'date',
				...
				$scope.getPagedDataAsync = function (pageSize, page,
					...
					setTimeout(
						function () {
							var data;
							var bonInventaireCourante = $scope.bonTransfertReceptionCourante;
							bonInventaireCourante.type = 'Reception';
							if (searchText) {
								var ft = searchText
									.toLowerCase();
