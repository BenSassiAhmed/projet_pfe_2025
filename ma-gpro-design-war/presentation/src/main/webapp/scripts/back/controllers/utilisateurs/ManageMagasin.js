'use strict'
angular
		.module('gpro.manageMagasin', [ "ngResource" ])
		.controller(
				'manageMagasinController',
				[
						'$scope',
						'$filter',
						'$http',
						...
						function($scope, $filter, $http, $log, downloadService, UrlCommun,UrlAtelier) {
							...
							$scope.listePartieInteressee = function() {
								$http.get(UrlCommun + "/magasin/all")
											...
											function() {
												$scope.colDefs = [
														...
														
														{
															...
															displayName : 'Depot',
															width:'8%',
															cellTemplate:`<div ng-if="row.entity.depot == true">OUI</div>
																          <div ng-if="row.entity.depot == false">NON</div>`
														},
														
														
														{
															field : 'pointVente',
															displayName : 'Point Vente',
															width:'10%',
															cellTemplate:`<div ng-if="row.entity.pointVente == true">OUI</div>
																          <div ng-if="row.entity.pointVente == false">NON</div>`
														},
													     {
												              field: '',
												              //	width: '10%',
