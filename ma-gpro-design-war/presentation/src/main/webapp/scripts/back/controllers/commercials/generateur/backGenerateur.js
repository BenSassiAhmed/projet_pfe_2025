...
var app = angular.module('gpro.back-generateur', [ "ngResource" ]);
app.controller('BackGenerateurCtr', [ '$scope','UrlAtelier','$log', function($scope,UrlAtelier,$log) {
...
//		    //Lister Sequence annuel
		    ...
		    //	$http.get(UrlAtelier+"/guichet/getById:"+3).success(function (dataSequence) {
		    	...
		    	$http.get(UrlAtelier+"/guichet/getAnneCourant").success(function (dataSequence) {
		    		
		    		$scope.sequenceCourante = dataSequence;
		    	//	console.log("dataSequence "+ dataSequence.annee+"---"+dataSequence.numReferenceFactureCourant);
		    		//if(dataSequence.length>0)
		  
		    	/*	$scope.sequenceCourante.id=	dataSequence.id;	    		
		    		...
		    		$scope.sequenceCourante.numReferenceBonLivraisonCourante=dataSequence.numReferenceBonLivraisonCourante;
		    		
		    		
		    		$scope.sequenceCourante.prefixeAvoir=dataSequence.prefixeAvoir;
		    		$scope.sequenceCourante.prefixeFacture=dataSequence.prefixeFacture;
		    		
		    		$scope.sequenceCourante.annee=dataSequence.annee;
		    		$scope.sequenceCourante.numReferenceCommandeCourante=dataSequence.numReferenceCommandeCourante;
		    		*/
		  	...
		  	$scope.saveSequence = function(sequence) {
		  		...
		  		console.log("seqence ReferenceAvoir: "+sequence.numReferenceAvoirCourante);
		  		console.log("ReferenceCommande"+sequence.numReferenceCommandeCourante);
			  
				     $http.post(UrlAtelier + "/guichet/updateGuichet", sequence)
